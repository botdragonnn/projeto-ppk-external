...
#define IMGUI_IGNORE_PRAGMA_WARNINGS

// Bring ImGui symbols into scope for this translation unit only
...
namespace edited
{
    ...
    bool BeginPopupW(const char* id, const char* hint)
    {
        ...
        if (ImGui::IsItemHovered() && hint[0]) SetTooltip(hint);
    ...
    bool Pages(bool selected, const char* label, ImTextureID image_icon, const ImVec2& size_arg)
    {
        ...
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(c::element::page_active, state.alpha), GetStyle().FrameRounding);
...
#include <string>
    ...
    bool Keybind(const char* label, int* key, int* mode, bool* show)
    {
        ...
        window->DrawList->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::element::popup_elements::filling), GetStyle().FrameRounding);
    ...
    bool BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, ImGuiChildFlags child_flags, ImGuiWindowFlags window_flags)
    {
        ...
        GetWindowDrawList()->AddRectFilled(parent_window->DC.CursorPos - ImVec2(0, 35), parent_window->DC.CursorPos + ImVec2(size_arg.x - 0.4f, 0), GetColorU32(c::accent), GetStyle().FrameRounding, ImDrawFlags_RoundCornersTop);
        GetWindowDrawList()->AddRectFilled(parent_window->DC.CursorPos - ImVec2(0, 34), parent_window->DC.CursorPos + ImVec2(size_arg.x, 0), GetColorU32(c::child::top), GetStyle().FrameRounding, ImDrawFlags_RoundCornersTop);
        ...
        parent_window->DrawList->AddRectFilled(child_window->Pos, child_window->Pos + ImVec2(size_arg.x, state.size_offset), GetColorU32(c::child::filling), GetStyle().FrameRounding, ImDrawFlags_RoundCornersBottom);
    ...
    bool button(bool select, const char* label, const ImVec2& size_arg)
    {
        ...
        state.background = ImLerp(state.background, pressed ? c::accent : c::element::popup_elements::filling, g.IO.DeltaTime * 6.f);
    ...
    bool color_button(const char* name, ImVec2 size, ImU32 color_bg)
    {
        ...
        GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(color_bg), GetStyle().FrameRounding);
    ...
    bool icon_box(const char* icon, ImVec2 size, ImU32 color_bg, ImU32 color_icon, ImU32 color_border)
    {
        ...
        ImGui::GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(color_bg), GetStyle().FrameRounding);
        ImGui::GetWindowDrawList()->AddRect(rect.Min, rect.Max, GetColorU32(color_border), GetStyle().FrameRounding);
    ...
    bool BeginCombo(const char* label, const char* preview_value, int val, bool multi, ImGuiComboFlags flags)
    {
        ...
        state.combo_size = ImLerp(state.combo_size, state.opened_combo ? (val == 166 ? 200 : CalcMaxPopupHeightFromItemCount(val)) : 0.f, g.IO.DeltaTime * 12.f);

        GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::element::combo::filling), GetStyle().FrameRounding);
        ...
        PushStyleVar(ImGuiStyleVar_WindowRounding, GetStyle().FrameRounding);
        PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(15, 15));
    ...
    bool Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
    {
        ...
        GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max + ImVec2(0, 1), GetColorU32(c::element::selectable, state.opticaly), GetStyle().FrameRounding);
    ...
    bool info_box(const char* icon, const char* label, ImVec2 size)
    {
        ...
        GetWindowDrawList()->AddRectFilled(rect.Min, rect.Max, GetColorU32(c::element::info_bar::box), GetStyle().FrameRounding);
        GetWindowDrawList()->AddRect(rect.Min, rect.Max, GetColorU32(c::element::info_bar::box_outline), GetStyle().FrameRounding);
...
// ImGui::CustomChild and ImGui::EndCustomChild are defined in imgui.cpp
// ImGui::KeyBind and ImGui::TabHeader are declared in imgui.h