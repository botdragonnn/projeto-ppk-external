...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    if (size.x <= 0.f && size.y <= 0.f)
        size = ImVec2(window->Size.x - 20, 25);
    else
    {
        if (size.x <= 0.f)
            size.x = label_size.x + style.FramePadding.x * 2.0f;
        if (size.y <= 0.f)
    ...
    static std::unordered_map<ImGuiID, BtnAnim> s_btnAnim;
    BtnAnim& anim = s_btnAnim[id];

    const float targetRise = held ? 1.f : (hovered ? 1.f : 0.f);
    const float riseSpeed = held ? 22.f : (hovered ? 16.f : 9.f);
    anim.fillRise = ImLerp(anim.fillRise, targetRise, ImClamp(g.IO.DeltaTime * riseSpeed, 0.f, 1.f));

    const float targetLift = held ? -3.f : (hovered ? -1.75f : 0.f);
    anim.lift = ImLerp(anim.lift, targetLift, ImClamp(g.IO.DeltaTime * 18.f, 0.f, 1.f));

    const ImVec2 visualOff(0.f, anim.lift);
    const float alpha = GetStyle().Alpha;
    const int baseA = (int)(alpha * 255);

    const ImRect drawBb(bb.Min + visualOff, bb.Max + visualOff);
    window->DrawList->AddRectFilled(drawBb.Min, drawBb.Max, IM_COL32(14, 14, 14, baseA), g.Style.FrameRounding);

    if (anim.fillRise > 0.002f)
    {
    ...
    window->DrawList->AddRect(drawBb.Min, drawBb.Max, GetColorU32(ImGuiCol_Border), g.Style.FrameRounding);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");
    RenderTextClipped(drawBb.Min + style.FramePadding, drawBb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &drawBb);

    IMGUI_TEST_ENGINE_ITEM_INFO(id, label, g.LastItemData.StatusFlags);
    return pressed;
}

ImColor GetPrimaryColor(int alpha = 255)
{
    return ImColor(
        (int)(g_Options.General.PrimaryColor[0] * 255),
        (int)(g_Options.General.PrimaryColor[1] * 255),
        (int)(g_Options.General.PrimaryColor[2] * 255),
        (int)(g_Options.General.PrimaryColor[3] * alpha)
    );
}

// Função para a cor de fundo dos widgets (versão escura da cor primária)
ImColor GetWidgetBackgroundColor(int alpha = 155)
{
    // Cria uma versão mais escura da cor primária para fundos
    float darkenFactor = 0.3f; // Escurecer 30%
    return ImColor(
        (int)(g_Options.General.PrimaryColor[0] * 255 * darkenFactor),
        (int)(g_Options.General.PrimaryColor[1] * 255 * darkenFactor),
        (int)(g_Options.General.PrimaryColor[2] * 255 * darkenFactor),
        (int)(g_Options.General.PrimaryColor[3] * alpha)
    );
}
...
bool ImGui::Tab(const char* Label, const char* Icon, bool Enabled)
{
    ...
    const ImVec2 label_size_bold = CalcTextSize(Label, NULL, true);
    PopFont();

    const ImVec2 pos = window->DC.CursorPos;
    float icon_width = CalcTextSize(Icon).x;
    float text_offset = (icon_width > 0) ? 32.f : 16.f;
    const ImRect total_bb(pos, pos + ImVec2(label_size_bold.x + text_offset + 20.f, label_size_bold.y + 18.f));
    ...
    static float anim_rel_min_x = 0;
    static float anim_rel_max_x = 0;
    static ImGuiID last_window_id = 0;

    if (Enabled)
    {
        float target_rel_min_x = total_bb.Min.x - window->Pos.x + 6.f;
        float target_rel_max_x = total_bb.Max.x - window->Pos.x - 6.f;

        if (last_window_id != window->ID || (anim_rel_min_x == 0 && anim_rel_max_x == 0)) {
            anim_rel_min_x = target_rel_min_x;
            anim_rel_max_x = target_rel_max_x;
        ...
        if (icon_width > 0)
        {
            ...
            window->DrawList->AddText(total_bb.Min + ImVec2(12.f - icon_width / 2.f, total_bb.GetHeight() / 2.f - CalcTextSize(Icon).y / 2.f), ImColor(1.f, 1.f, 1.f, GetStyle().Alpha), Icon);
            PopFont();
        }
        window->DrawList->AddText(total_bb.Min + ImVec2(text_offset, total_bb.GetHeight() / 2.f - CalcTextSize(Label).y / 2.f), ImColor(1.f, 1.f, 1.f, GetStyle().Alpha), Label);
        PopFont();
    }
    else
    {
        const ImU32 colInactive = IM_COL32(152, 152, 158, (int)(GetStyle().Alpha * 255));
        PushFont(FrameWork::Assets::InterMedium);
        const ImVec2 label_sz = CalcTextSize(Label, NULL, true);
        if (icon_width > 0)
        {
            PushFont(FrameWork::Assets::FontAwesomeSolid);
            window->DrawList->AddText(total_bb.Min + ImVec2(12.f - icon_width / 2.f, total_bb.GetHeight() / 2.f - CalcTextSize(Icon).y / 2.f), IM_COL32(152, 152, 158, (int)(GetStyle().Alpha * 255)), Icon);
            PopFont();
        }
        window->DrawList->AddText(total_bb.Min + ImVec2(text_offset, total_bb.GetHeight() / 2.f - label_sz.y / 2.f), colInactive, Label);
        PopFont();
    }
...
static std::unordered_map<int, TabHeaderAnim> TabHeaderStates;
...
bool ImGui::TabHeaderButton(const char* ButtonName, bool enabled, int direction, int TabIndex)
{
    ...
    auto Value = TabHeaderStates.find(TabIndex);
    if (Value == TabHeaderStates.end())
    {
        TabHeaderStates.insert({ TabIndex, TabHeaderAnim() });
        Value = TabHeaderStates.find(TabIndex);
    }

    if (enabled)
    {
        Value->second.selectedpox = pos.x;
        Value->second.target_width = size.x;
    }
    ...
    Value->second.posx = ImLerp(Value->second.posx, Value->second.selectedpox - window->Pos.x, 0.15f);
    Value->second.anim_width = ImLerp(Value->second.anim_width, Value->second.target_width, 0.15f);
...
bool ImGui::CfgButton(const char* Label, std::function<void()> Components)
{
    ...
    ImVec2 IconTextSize = CalcTextSize(FrameWork::Assets::FontAwesomeSolid, FrameWork::Assets::FontAwesomeSolid->FontSize - 4, ICON_FA_GEAR); //ICON_FA_ELLIPSIS
    ...
    CfgAnim->second.IconColor = ImLerp(CfgAnim->second.IconColor, CfgAnim->second.PopupActive ? ImColor(Base) : ImColor(60, 60, 60), g.IO.DeltaTime * 10.f);

    ImRotateStart();
    Window->DrawList->AddText(FrameWork::Assets::FontAwesomeSolid, FrameWork::Assets::FontAwesomeSolid->FontSize - 4, Pos, ImGui::GetColorU32(CfgAnim->second.IconColor), ICON_FA_GEAR); //ICON_FA_ELLIPSIS
    ImRotateEnd(1.57f * CfgAnim->second.GearRotation);

    CfgAnim->second.PopupAlpha = ImClamp(CfgAnim->second.PopupAlpha + (6.f * g.IO.DeltaTime * (CfgAnim->second.PopupActive ? 1.f : -1.f)), 0.f, 1.f);
    ...
    if (CfgAnim->second.PopupAlpha >= 0.001f) {
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(ImVec4(ImColor(20, 20, 24))));
...
bool ImGui::WeaponButtonHeld(ImTextureID Icon, const char* label, ImGuiButtonFlags flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    if (Pressed) {
        WeaponButtonAnim.Active = true;
    }

    if (WeaponButtonAnim.Active) {
        WeaponButtonAnim.Progress = ImClamp(WeaponButtonAnim.Progress + g.IO.DeltaTime, 0.f, 1.f);
    }

    if (WeaponButtonAnim.Progress >= 1.f) {
        WeaponButtonAnim.Active = false;
        WeaponButtonAnim.Progress = 0.f;
        return true;
    }

    // Desenha o botão de fundo
    Draw->AddRectFilled(Rect.Min, Rect.Max, GetColorU32(ImVec4(0.1f, 0.1f, 0.1f, 1.0f)), 8);

    // Desenha o ícone no centro
    float Padding = 10.0f;
    Draw->AddImage(Icon, ImVec2(Rect.Min.x + Padding, Rect.Min.y + Padding), ImVec2(Rect.Max.x - Padding, Rect.Max.y - Padding));

    if (WeaponButtonAnim.Active) {
        float ProgressHeight = Height * WeaponButtonAnim.Progress;
        Draw->AddRectFilled(Pos + ImVec2(0, Height - ProgressHeight), Pos + ImVec2(Width, Height), GetColorU32(ImVec4(0.5f, 0.7f, 1.0f, 0.8f)), 8);
    }
...
bool ImGui::TabHeader(const char* tabname, int* currentTab, std::vector<const char*> Tabs, int TabIndex)
{
    ...
    ImVec2 start_cursor_pos = window->DC.CursorPos - window->Pos; // relative pos
    ...
    ImVec2 size = ImVec2(WidhtMax, 40);
    ...
    // Draw the main container background with a border
    ImVec2 bg_min = GetWindowPos() + GetCursorPos();
    ImVec2 bg_max = bg_min + ImVec2(WidhtMax, 40);
    
    // Dark background
    window->DrawList->AddRectFilled(bg_min, bg_max, ImColor(18, 18, 18, (int)(GetStyle().Alpha * 255)), g.Style.FrameRounding, ImDrawFlags_RoundCornersAll);
    // Slight border
    window->DrawList->AddRect(bg_min, bg_max, ImColor(35, 35, 35, (int)(GetStyle().Alpha * 255)), g.Style.FrameRounding, ImDrawFlags_RoundCornersAll);

    // Get the animation state for this header
    auto Value = TabHeaderStates.find(TabIndex);
    if (Value != TabHeaderStates.end())
    {
        // Draw the animated background highlight (using a subtle gray for active state)
        ImVec2 highlight_min = ImVec2(window->Pos.x + Value->second.posx + 2, bg_min.y + 2);
        ImVec2 highlight_max = ImVec2(window->Pos.x + Value->second.posx + Value->second.anim_width - 2, bg_max.y - 2);
        window->DrawList->AddRectFilled(highlight_min, highlight_max, ImColor(45, 45, 45, (int)(GetStyle().Alpha * 200)), g.Style.FrameRounding, ImDrawFlags_RoundCornersAll);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float circleR = 6.f;
    const float circleD = circleR * 2.f;
    const float padX = 4.f;

    const ImVec2 pos = window->DC.CursorPos;
    const float contentW = GetWindowSize().x - 20;
    const ImRect total_bb(pos, pos + ImVec2(contentW, rowH));
    ...
    Value->second.a = ImLerp(Value->second.a, *v ? 1.f : 0.f, g.IO.DeltaTime * 12.f);

    // Toggle switch position (right side)
    const float toggleX = total_bb.Max.x - toggleW;
    const float toggleY = pos.y + (rowH - toggleH) * 0.5f;
    const ImRect toggleBb(ImVec2(toggleX, toggleY), ImVec2(toggleX + toggleW, toggleY + toggleH));
    ...
    // Border
    window->DrawList->AddRect(toggleBb.Min, toggleBb.Max, IM_COL32(50, 50, 50, 255), toggleH * 0.5f);

    // Sliding circle
    const float circleX = toggleBb.Min.x + padX + (toggleW - circleD - padX * 2) * Value->second.a;
    const float circleY = toggleBb.Min.y + (toggleH - circleD) * 0.5f;
    ImVec4 cA(80 / 255.f, 80 / 255.f, 80 / 255.f, 1.f);
    ImVec4 cB(1.f, 1.f, 1.f, 1.f);
    ImVec4 cLerp = ImLerp(cA, cB, Value->second.a);
    window->DrawList->AddCircleFilled(ImVec2(circleX + circleR, circleY + circleR), circleR, GetColorU32(cLerp));

    // Text label
    if (label_size.x > 0.0f)
        RenderText(ImVec2(total_bb.Min.x, total_bb.Min.y + (rowH - label_size.y) * 0.5f), label);
...
bool ImGui::KeyBind(const char* label, int* Key, int* KeyState)
{
    ...
    ImGui::PushFont(FrameWork::Assets::InterRegular);
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(14, 14, 14), g.Style.FrameRounding);
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags, int itemcount)
{
    ...
    {
        window->DrawList->AddRectFilled(bb.Min, bb.Max, ImColor(14, 14, 14, (int)(GetStyle().Alpha * 255)), g.Style.FrameRounding);
        window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(ImGuiCol_Border), g.Style.FrameRounding);
        window->DrawList->AddCircleFilled(ImVec2(bb.Max.x - 14, bb.Min.y + total_bb.GetHeight() / 2), Value->second.a, ImColor(23, 24, 25, (int)(GetStyle().Alpha * 155)), 360);
    }
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags, int itemcount)
{
    ...
    PushStyleColor(ImGuiCol_PopupBg, ImColor(20, 20, 24).Value);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x && Value->second.anim > 2.f)
    {
    ...
    {
        const ImVec2 grabMin(frame_bb.Min.x + Value->second.anim - 2.f, frame_bb.Min.y);
        const ImVec2 grabMax(frame_bb.Min.x + Value->second.anim + 2.f, frame_bb.Max.y);
        const int grabGlowA = (int)(GetStyle().Alpha * 90.f);
        window->DrawList->AddShadowRect(grabMin, grabMax, IM_COL32(255, 255, 255, grabGlowA), 10.f, ImVec2(0.f, 0.f), 0, 2.f);
        window->DrawList->AddRectFilled(grabMin, grabMax, ImColor(255, 255, 255, (int)(0.95f * GetStyle().Alpha * 255)), 5);
    }
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    // ✅ MODIFICAÇÃO: Forçar NoInputs para remover caixas de texto
    ...
    // ✅ MODIFICAÇÃO: Removida toda a seção de inputs RGB/HSV/Hex
    // Apenas o color button e picker serão mostrados

    ImGuiWindow* picker_active_window = NULL;
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        const float button_offset_x = 0.0f; // ✅ MODIFICAÇÃO: Sem inputs, sempre na esquerda
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        window->DC.CursorPos.x = pos.x + w_button; // ✅ MODIFICAÇÃO: Ajustado para apenas o botão
        TextEx(label, label_display_end);
    }
...
void ImGui::TabBarQueueReorderFromMousePos(ImGuiTabBar* tab_bar, ImGuiTabItem* src_tab, ImVec2 mouse_pos)
{
    ...
    for (int i = src_idx; i >= 0 && i < tab_bar->Tabs.Size; i += dir)
    {
        ...
        //GetForegroundDrawList()->AddRect(ImVec2(x1, tab_bar->BarRect.Min.y), ImVec2(x2, tab_bar->BarRect.Max.y), IM_COL32(255, 23, 24, 25));
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    if (out_text_clipped)
    {
        *out_text_clipped = (text_ellipsis_clip_bb.Min.x + label_size.x) > text_pixel_clip_bb.Max.x;
        //draw_list->AddCircle(text_ellipsis_clip_bb.Min, 3.0f, *out_text_clipped ? IM_COL32(255, 23, 24, 25) : IM_COL32(0, 255, 0, 255));
    }
