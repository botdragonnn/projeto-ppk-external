...
{
	// Let all keys pass through naturally to the foreground window.
	// When overlay is foreground (menu open), keys arrive via normal window messages.
	// When game is foreground (menu closed), keys go to the game.
...
void CreateDeviceD3D()
{
	...
	{
#ifdef _DEBUG
		std::cout << XorStr("[ERROR] D3D11CreateDeviceAndSwapChain") << std::endl;
#endif
		return;
	}
...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	// DefWindowProc does nothing useful for these messages on our popup overlay
	switch (uMsg)
	{
	case WM_KEYDOWN:
	case WM_KEYUP:
...
namespace FrameWork
{
	...
	void Overlay::Initialize()
	{
		...
		{
#ifdef _DEBUG
			std::cout << XorStr("[ERROR] Overlay Not Settuped!") << std::endl;
#endif
			return;
		}
		...
		{
#ifdef _DEBUG
			std::cout << XorStr("[ERROR] RegisterClassEx") << std::endl;
#endif
			return;
		}

		hWindow = CreateWindowEx(WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_TOOLWINDOW,
		...
		{
#ifdef _DEBUG
			std::cout << XorStr("[ERROR] CreateWindowEx") << std::endl;
#endif
			return;
		}
